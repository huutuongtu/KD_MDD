...
for epoch in range(num_epoch):
  ...
  for i, data in tqdm(enumerate(train_loader)):
    ...
    loss = 0.5*ctc_loss(logits, labels, input_lengths, target_lengths) + (kd_loss(s1, logits_teacher, KD_inputlengths, temperature=5) + kd_loss(s2, logits_teacher, KD_inputlengths, temperature=5) + kd_loss(s3, logits_teacher, KD_inputlengths, temperature=5))/6
  ...
  if epoch>=7:
    with torch.no_grad():
      ...
      if (epoch_wer < min_wer):
        ...
        min_wer = epoch_wer
        torch.save(student.state_dict(), 'checkpoint/improve_inter_kd_XLSR_w2v2_student.pth')
