...
from MDD_model import Wav2Vec2_Student, Wav2Vec2_Student_woL
from pyctcdecode import build_ctcdecoder
from jiwer import wer
import ast
...
model = Wav2Vec2_Student_woL.from_pretrained(
  ...
  if epoch>=7:
    with torch.no_grad():
      ...
      if (epoch_wer < min_wer):
        ...
        min_wer = epoch_wer
        torch.save(model.state_dict(), 'wol_XLSR_w2v2_student.pth')
      # with open('wer_student.txt', 'a') as wer_file:
