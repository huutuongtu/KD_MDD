...
for epoch in range(num_epoch):
  ...
  for i, data in tqdm(enumerate(train_loader)):
    ...
    output = labels
    transcript = labels
    s1, s2, s3, logits= student(acoustic)
    t1, t2, t3, t4, t5, t6, t7, t8, logits_teacher = teacher(acoustic)
    L_KD = mse_loss(F.softmax(logits, dim=2), F.softmax(logits_teacher, dim=2), KD_inputlengths) + mse_loss(F.softmax(s1, dim=2), F.softmax(logits_teacher, dim=2), KD_inputlengths) + mse_loss(F.softmax(s2, dim=2), F.softmax(logits_teacher, dim=2), KD_inputlengths) + mse_loss(F.softmax(s3, dim=2), F.softmax(logits_teacher, dim=2), KD_inputlengths)

    logits = logits.transpose(0,1)
    s1, s2, s3 = s1.transpose(0,1), s2.transpose(0,1), s3.transpose(0,1)
    ...
    s1, s2, s3 = F.log_softmax(s1, dim=2), F.log_softmax(s2, dim=2), F.log_softmax(s3, dim=2)

    L_CTC = ctc_loss(logits, labels, input_lengths, target_lengths) + ctc_loss(s1, labels, input_lengths, target_lengths) + ctc_loss(s2, labels, input_lengths, target_lengths) + ctc_loss(s3, labels, input_lengths, target_lengths)
    
    loss = L_CTC + 0.25*L_KD
  ...
  if epoch>=7:
    with torch.no_grad():
      ...
      if (epoch_wer < min_wer):
        ...
        min_wer = epoch_wer
        torch.save(student.state_dict(), 'checkpoint/inter_kd_XLSR_w2v2_student.pth')
